...
import { ArrowUpRight, Zap, Sparkles, Shield, ChevronRight, Check } from 'lucide-react';
...
export const EvolutionModal: React.FC<EvolutionModalProps> = ({
  ...
  const levelUpCost = Math.floor(200 * Math.pow(1.18, progress.level - 1));
      ...
      <div className="w-full max-w-2xl rounded-3xl bg-slate-900 border-2 border-slate-700 shadow-2xl overflow-hidden text-slate-100 flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-amber-400">{unit.baseName}</h2>
              <p className="text-xs text-slate-400">独自進化＆育成ツリー</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          {/* XP & Resources Header */}
          <div className="flex items-center justify-between rounded-2xl bg-slate-950 p-3 border border-slate-800">
            <div className="flex items-center gap-2">
              <span className="text-sm font-black text-amber-400">Lv.{progress.level}</span>
              <span className="text-xs text-slate-400">現在ステージ: {progress.currentStage}形態</span>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {PASSIVE_SKILLS.map((skill) => {
                  ...
                  return (
                    <div
                      key={skill.id}
                      onClick={() => onEquipSkill(unit.id, skill.id)}
                    ...
                    >
                      ...
                      {isEquipped ? (
                        ...
                        <span className="text-[10px] font-bold text-slate-500">装着</span>
                      )}
                    </div>
                  );
                })}
